...
/// that routes STRM-prefixed streams to the native handler and everything
...
public class SurgewaveStreamHandlerTests
{
    ...
    public async Task HandleAsync_KafkaMagicBytes_ProcessesWithoutCrash()
    {
        ...
        // 4 non-STRM bytes followed by EOF → Kafka path reads size, finds
    ...
    [Fact]
    public async Task HandleAsync_StrmMagic_RoutesToNativeHandler()
    {
        await using var runtime = await Kuestenlogik.Surgewave.Runtime.SurgewaveRuntime.CreateBuilder()
            .WithPort(0)
            .WithStorageEngine(Kuestenlogik.Surgewave.Core.Storage.StorageEngines.Memory)
        ...
        // Verify STRM routing via a real loopback TCP connection.
        ...
        // Write STRM magic + version byte from client side
        var clientStream = client.GetStream();
        await clientStream.WriteAsync(new byte[] { 0x53, 0x54, 0x52, 0x4D, 0x01 });
