...
public sealed class ConsumerGroupV2CoordinatorTests : IDisposable
{
    ...
    public void Heartbeat_RebalanceTimeoutMs_OnlyShrinksAfterInit()
    {
        ...
        Assert.Equal(ErrorCode.None, first.ErrorCode);
    ...
    public void Heartbeat_FirstMember_GetsAllPartitionsOfSubscribedTopic()
    {
        ...
        Assert.NotNull(resp.MemberAssignment);
        var assignment = Assert.Single(resp.MemberAssignment.TopicPartitions);
    ...
    public void Heartbeat_SecondMember_TargetSplitButReconciliationWithholdsConflictingPartitions()
    {
        ...
        var ownedByFirst = first.MemberAssignment!.TopicPartitions
            .Select(t => new ConsumerGroupHeartbeatRequest.Assignor { TopicId = t.TopicId, Partitions = t.Partitions })
            .ToList();
        ...
        // models this as an absent (null) MemberAssignment in the response.
        Assert.Null(second.MemberAssignment);
    }
    ...
    public void Heartbeat_SecondMember_AfterFirstRevokes_GetsItsRangeShare()
    {
        ...
        SendHeartbeat("g3", "c1", subscribed: null, memberId: firstId, memberEpoch: first.MemberEpoch,
            owned: AssignmentToOwned(first.MemberAssignment!));
        ...
        var refetched = SendHeartbeat("g3", "c1", subscribed: null, memberId: firstId,
            ...
            owned: [new ConsumerGroupHeartbeatRequest.Assignor
            {
                TopicId = _topicAId,
                Partitions = [0, 1, 2],
            }]);
        ...
        Assert.NotNull(second2.MemberAssignment);
        var assignment = Assert.Single(second2.MemberAssignment.TopicPartitions);
        Assert.Equal(_topicAId, assignment.TopicId);
        Assert.Equal(new[] { 3, 4, 5 }, assignment.Partitions);
        Assert.Equal(new[] { 0, 1, 2 }, refetched.MemberAssignment!.TopicPartitions[0].Partitions);
    }
    ...
    public void Heartbeat_LeaveRequest_RemovesMemberAndRebalances()
    {
        ...
        var leaveResp = _coordinator.HandleConsumerGroupHeartbeat(new ConsumerGroupHeartbeatRequest
        {
    ...
    public void Heartbeat_ServerAssignorRoundRobin_DistributesAcrossTopics()
    {
        ...
        var describe = _coordinator.HandleConsumerGroupDescribe(new ConsumerGroupDescribeRequest
        {
    ...
    public void Heartbeat_UnknownServerAssignor_KeepsRangeAsDefault()
    {
        ...
        Assert.NotNull(first.MemberAssignment);
    }
    ...
    public void Describe_ReflectsReconcilingAndStableStates()
    {
        ...
        Assert.Equal("Stable", stable.Members.Count == 0 ? "Empty" : stable.GroupState);
        ...
        Assert.Equal("Reconciling", reconciling.GroupState);
    }

    private ConsumerGroupHeartbeatResponse SendHeartbeat(
        string group,
        string clientId,
        List<string>? subscribed,
        string? memberId = null,
        int memberEpoch = 0,
        List<ConsumerGroupHeartbeatRequest.Assignor>? owned = null,
        string? serverAssignor = null,
        int rebalanceTimeoutMs = -1)
    {
        return _coordinator.HandleConsumerGroupHeartbeat(new ConsumerGroupHeartbeatRequest
        {
            ApiKey = ApiKey.ConsumerGroupHeartbeat,
            ApiVersion = 0,
            CorrelationId = 0,
    ...
    private ConsumerGroupDescribeResponse.DescribedGroup Describe(string groupId)
    {
        var resp = _coordinator.HandleConsumerGroupDescribe(new ConsumerGroupDescribeRequest
        {
            ApiKey = ApiKey.ConsumerGroupDescribe,
            ApiVersion = 0,
            CorrelationId = 0,
        ...
        return Assert.Single(resp.Groups);
    }

    private static List<ConsumerGroupHeartbeatRequest.Assignor> AssignmentToOwned(
        ConsumerGroupHeartbeatResponse.Assignment assignment)
    {
        var result = new List<ConsumerGroupHeartbeatRequest.Assignor>(assignment.TopicPartitions.Count);
        foreach (var t in assignment.TopicPartitions)
        {
            result.Add(new ConsumerGroupHeartbeatRequest.Assignor
            {
                TopicId = t.TopicId,
