...
public class CliIntegrationTests : IAsyncLifetime
{
    ...
    public async Task Topics_CreateListDescribeDelete_Workflow()
    {
        ...
        // Delete topic — pass --yes so the destructive-confirmation prompt
        // does not stall the non-interactive CI runner (CLI prompts default
        // to `n` when stdin is unavailable, blocking the test indefinitely).
        var (deleteExit, deleteOutput) = await RunCliAsync($"topics delete {topicName} --yes");
