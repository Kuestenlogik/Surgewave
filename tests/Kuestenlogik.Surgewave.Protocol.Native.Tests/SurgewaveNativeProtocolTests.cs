...
public sealed class SurgewaveNativeProtocolTests
{
    ...
    [Fact]
    public void Magic_Is_STRM()
    {
        // Verify magic bytes are "STRM"
        Assert.Equal(4, SurgewaveNativeProtocol.Magic.Length);
        Assert.Equal((byte)'S', SurgewaveNativeProtocol.Magic[0]);
        Assert.Equal((byte)'T', SurgewaveNativeProtocol.Magic[1]);
        Assert.Equal((byte)'R', SurgewaveNativeProtocol.Magic[2]);
        Assert.Equal((byte)'M', SurgewaveNativeProtocol.Magic[3]);
    }
