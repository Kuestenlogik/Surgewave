...
/// <summary>
/// Benchmark for Kafka client (Confluent.Kafka) against Surgewave broker using Kafka protocol.
/// This measures Surgewave's Kafka protocol compatibility performance.
/// </summary>
public static class KafkaClientToSurgewaveBenchmark
{
    private const string BootstrapServers = "localhost:9092";
    private const int DefaultMessageCount = 100_000;
    private const int DefaultMessageSize = 100;
    ...
    public static async Task RunAsync(string[] args)
    {
        ...
        Console.WriteLine("╔════════════════════════════════════════════════════════════════╗");
        Console.WriteLine("║     KAFKA CLIENT → Surgewave BROKER BENCHMARK                      ║");
        Console.WriteLine("║     (Confluent.Kafka client using Kafka protocol)              ║");
        Console.WriteLine("╚════════════════════════════════════════════════════════════════╝");
        Console.WriteLine();
        ...
        var topicName = $"kafka-surgewave-bench-{Guid.NewGuid():N}";
        ...
        Console.WriteLine("═══════════════════════════════════════════════════════════════════");
        var syncTopicName = $"kafka-surgewave-sync-{Guid.NewGuid():N}";
        ...
        {
            BootstrapServers = BootstrapServers,
            GroupId = $"kafka-surgewave-consumer-{Guid.NewGuid():N}",
        ...
        Console.WriteLine();
        Console.WriteLine("═══════════════════════════════════════════════════════════════════");
        Console.WriteLine("  KAFKA CLIENT → Surgewave BROKER SUMMARY");
        Console.WriteLine("═══════════════════════════════════════════════════════════════════");
