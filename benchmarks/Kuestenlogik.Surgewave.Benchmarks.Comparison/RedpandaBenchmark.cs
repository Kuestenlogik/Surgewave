...
/// <summary>
/// Competitor-first Redpanda benchmark.
///
/// Default: benchmarks only Redpanda and saves the result to
///   <c>artifacts/benchmarks/results/redpanda.json</c>.
///
...
///
/// Run:
///   dotnet run -- benchmark-redpanda [msgCount] [msgSize] [bootstrap] [--include-surgewave]
/// </summary>
public static class RedpandaBenchmark
{
    public static async Task RunAsync(string[] args)
    {
        ...
        var redpandaBootstrap = args.Length > 2 && !args[2].StartsWith('-') ? args[2] : "localhost:19092";
        var includeSurgewave     = args.Any(a => a.Equals("--include-surgewave", StringComparison.OrdinalIgnoreCase));

        Console.WriteLine("╔══════════════════════════════════════════════════════════════════════════╗");
        Console.WriteLine("║                      REDPANDA BENCHMARK                                  ║");
        Console.WriteLine("╠══════════════════════════════════════════════════════════════════════════╣");
        Console.WriteLine("║  Redpanda broker + Confluent.Kafka client                                ║");
        if (includeSurgewave)
        ...
        Console.WriteLine($"  Redpanda broker:  {redpandaBootstrap}");
        Console.WriteLine();
        ...
        // ── Redpanda ───────────────────────────────────────────────────────
        Console.WriteLine("── Redpanda (Redpanda + Confluent.Kafka) ───────────────────────────────────");
        try
        {
            var result = await ComparisonHelper.RunKafkaBenchmarkAsync(
                redpandaBootstrap, messageCount, messageSizeBytes, messageValue, "redpanda");
        ...
        {
            Console.WriteLine($"  SKIPPED: Redpanda broker not available at {redpandaBootstrap}");
            Console.WriteLine($"  Error:   {ex.Message}");
            Console.WriteLine($"  Tip:     docker run -d --name redpanda -p 19092:19092 redpandadata/redpanda:latest redpanda start --overprovisioned --smp 1 --kafka-addr 0.0.0.0:19092");
        }
