...
param(
    ...
    [switch]$Serve,
    [switch]$Local,
    [string]$BaseUrl = '/Surgewave'
)

# -Local switches the baseurl to '/' so the combined output is browsable
# from http://localhost:4000/ directly (instead of /Surgewave/). Jekyll's option
# parser rejects an empty string for --baseurl, so '/' is the canonical
# root-relative form. Use this for local preview; the production build
# keeps /Surgewave because GitHub Pages deploys the site under
# https://kuestenlogik.github.io/Surgewave/.
if ($Local) { $BaseUrl = '/' }
