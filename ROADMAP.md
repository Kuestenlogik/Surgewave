...
| G18 | **Basic AI primitives shipped as a default-bundled Apache-2.0 plugin** — five basic AI pipeline nodes (`LlmChatNode`, `EmbeddingNode`, `VectorStoreReadNode`, `VectorStoreWriteNode`, `PromptTemplateNode`) plus a `SingleShotRagNode` (retrieve→augment→generate in one node) stay in `Surgewave.Ai` under Apache-2.0 but ship in the official broker bundle so a minimal AI pipeline runs without an extra `surgewave plugin install` step. Keeps the core free of OpenAI/Anthropic/vector-store SDKs and their release cadence, while preserving the "AI out of the box" UX. Advanced AI (agent framework, multi-step RAG orchestration, guardrails, document processing, pipeline-chat UI, streaming-chat infrastructure, RAG eval, multimodal) stays in the separately-licensed AI extension. | ~2–3 days | Medium-High |
| ~~G20~~ | ~~**Adaptive compression per topic — broker integration**~~ ✅ **Done (2026-06-05)** — `SurgewaveAdaptiveCompressionBrokerPlugin` + `AdaptiveCompressionService` (`BackgroundService`) verdrahten `compression.type=auto`: periodischer Out-of-Band-Scan über `LogManager.ListTopics()`, per-Topic `AdaptiveCompressionSampler`, Decision-Writeback via `LogManager.UpdateTopicConfig`. Produce-Hot-Path unverändert. Opt-in via `Surgewave:AdaptiveCompression:Enabled=true`. |  |  |
...
| ~~G27~~ | ~~**Cold-start auto-tune — service integration**~~ ✅ **Done (2026-06-06)** — `ColdStartAutoTuneService` (`BackgroundService`) hält den `ColdStartWorkloadProfiler`, wartet auf `ObservationWindow` (Default 24 h), ruft `ColdStartTuningRecommender`, schreibt Audit-Trail nach `auto-tuned.json`. `DataApiHandler.HandleProduceAsync` füttert den Profiler optional. Opt-in via `Surgewave:AutoTune:ColdStart:Enabled=true`, AutoApply via `…:AutoApply=true`. Replication-Hook bleibt für Folge-Iteration offen. |  |  |
| G28 | **Leader-reelection latency after broker shutdown** — `ClusterController.HandleBrokerFailedAsync` emits "Transferring leadership for N partitions" via the `OnBrokerFailed` heartbeat callback, but the new leader assignment doesn't propagate into the `Metadata` response within a reasonable timeframe under Linux-CI load (>60 s observed). `ReplicationTests.Cluster_BrokerShutdown_RemainingBrokersContinue` is currently skipped because of this — clients reconnect, librdkafka eventually refreshes metadata, but the gap is long enough to time out a `MessageTimeoutMs=30000` producer. Investigation: trace where the `PartitionState.LeaderBrokerId` write reaches the metadata-response path, add a metadata-version bump to force fresh fetches. | ~3–5 days | Medium |
