...
- **`IBrokerPlugin`** --- features that register DI services and optionally map HTTP endpoints. A plugin self-declares via `RequiresLicense` whether it needs an `ILicenseProvider` check before activation; the core does not enumerate which feature ids are licensed.
...
Plugins whose `RequiresLicense` is `true` are checked against the optional `ILicenseProvider` via `IsFeatureEnabled(plugin.FeatureId)`. If no licence provider is registered or the feature is not enabled, the plugin is skipped with a warning. Community plugins (`RequiresLicense = false`) load unconditionally.
