...
**Community Repositories (Apache 2.0):**
...
**Enterprise Repositories (Business Source License 1.1):**
...
`Kuestenlogik.Surgewave.Plugins` exposes only contracts (`IBrokerPlugin`, `IProtocolPlugin`, `IStorageEnginePlugin`, `IPlugin`) and the optional licence-gate interface (`ILicenseProvider`). It carries no constants for individual community or licensed plugins; every plugin owns its own `FeatureId` string in its own assembly.

`ILicenseProvider` is an optional interface. When registered, it exposes `Edition`, `LicensedTo`, `ExpiresAt`, and `IsFeatureEnabled(featureName)`. When not registered (`null`), the broker runs in community mode — community plugins load unconditionally and any plugin that declares `RequiresLicense = true` is skipped with a warning.

### Enforcement

Each `IBrokerPlugin` self-declares whether it needs a licence via `RequiresLicense` (default `false`). `BrokerPluginActivator.ActivatePlugins()` consults `ILicenseProvider.IsFeatureEnabled(plugin.FeatureId)` only for plugins where `RequiresLicense` is `true`; community plugins load unconditionally. Protocol plugins (`IProtocolPlugin`) are not licence-gated.

This means:
1. The broker binary contains zero licensed-extension code or knowledge.
2. Licensed `.swpkg` packages can be installed but will not activate without a valid licence provider that enables their feature id.
3. Community deployments carry no licensed-extension overhead --- not even inactive code paths.
