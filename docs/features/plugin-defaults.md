...
| Tier | Source | Owner |
|------|--------|-------|
| 1 (lowest)  | `pluginsettings.json` from `plugins/<id>/<file>` | plugin author |
| 2           | broker `appsettings.json` + `appsettings.{Env}.json` | operator |
| 3 (highest) | environment variables + command-line arguments | ops automation |
...
- Manifest schema: `schemas/plugin-manifest/v1.json`
- Architecture rationale: `ARCHITECTURE.md` §3 (`.Packaging` sibling)
