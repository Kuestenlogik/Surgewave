...
Compaction keeps only the latest record for each key. Records `A:1`, `A:2`
become obsolete once `A:3` is written; same for the `B` and `C` keys:

```mermaid
flowchart TB
  subgraph Before["Before compaction (8 records)"]
    direction LR
    A1["A:1"] --> B1["B:1"] --> A2["A:2"] --> C1["C:1"] --> B2["B:2"] --> A3a["A:3"] --> C2a["C:2"] --> B3a["B:3"]
  end
  subgraph After["After compaction (3 records, latest per key)"]
    direction LR
    A3b["A:3"] --> C2b["C:2"] --> B3b["B:3"]
  end
...
```mermaid
flowchart TB
  Active["Active segment (not compacted)"]
  Closed2["Closed segment 2 (eligible for compaction)"]
  Closed1["Closed segment 1 (already compacted)"]
  Active --> Closed2 --> Closed1
```
