...
/// <summary>
/// MessagePack deserializer with schema registry integration.
/// Reads Confluent wire format: [0x00 magic][4-byte schema ID][MessagePack payload]
/// </summary>
/// <typeparam name="T">The type to deserialize to.</typeparam>
public sealed class SchemaRegistryMessagePackDeserializer<T> : IAsyncDeserializer<T>
{
    private readonly MessagePackSerializerConfig _config;
    private readonly ConcurrentDictionary<int, string> _schemaCache = new();

    /// <summary>
    /// Create a new schema registry MessagePack deserializer.
    /// </summary>
    /// <param name="config">Deserializer configuration.</param>
    public SchemaRegistryMessagePackDeserializer(MessagePackSerializerConfig config)
    {
    ...
    public async ValueTask<T> DeserializeAsync(ReadOnlyMemory<byte> data, string topic, CancellationToken cancellationToken = default)
    {
        ...
        var result = global::MessagePack.MessagePackSerializer.Deserialize<T>(payload, cancellationToken: cancellationToken);

        if (result == null)
            throw new InvalidOperationException("MessagePack deserialization returned null");
