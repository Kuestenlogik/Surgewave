...
public class PurgeDlqCommand : CommandBase
{
    ...
    private async Task<int> ExecuteAsync(ParseResult parseResult, CancellationToken ct)
    {
        ...
        try
        {
            ...
            // Confirm unless --yes / --force (or global --yes)
            if (format != OutputFormat.Json && !System.Console.IsInputRedirected)
            {
                ...
                if (!ConfirmDestructive(parseResult, prompt, localYes))
                {
            ...
            if (deleteTopic)
            {
                ...
                {
                    Console.WriteLine($"deleted {topic} messages={totalMessages}");
                }
                else
                {
            ...
            else
            {
                ...
                {
                    Console.WriteLine($"purged {topic} messages={totalMessages} partitions={partitionCount}");
                }
                else
                {
