...
{
    public ConfigShowCommand() : base("show", "Show the CLI's own settings (broker, format, verbose, active profile) — see `config view` for the broker's effective appsettings.json")
    {
...
internal sealed class ProfileRemoveCommand : CommandBase
{
    ...
    public ProfileRemoveCommand() : base("remove", "Remove a profile")
    {
        ...
        {
            var name = parseResult.GetValue(_nameArg)!;
            if (!ConfirmDestructive(parseResult, $"Remove profile '[cyan]{name}[/]'?"))
            {
