...
public class DeleteGroupCommand : CommandBase
{
    ...
    private readonly Option<bool> _forceOpt = new("--force", "-f") { Description = "Force deletion without confirmation", DefaultValueFactory = _ => false };
    ...
    private async Task<int> ExecuteAsync(ParseResult parseResult, CancellationToken ct)
    {
        ...
        {
            var confirm = AnsiConsole.Confirm($"Delete consumer group '[cyan]{groupId}[/]'?", false);
            if (!confirm)
            {
