...
public sealed class BrokerConfig : IValidatableConfig
{
    ...
    // Enterprise plugin: Kuestenlogik.Surgewave.Agent (stateless auto-scaling agent mode)
    // AgentConfig is now owned by the Surgewave.Agent plugin

    // Enterprise plugin: Kuestenlogik.Surgewave.Replication
    // Cluster Linking settings (topic-level cross-cluster mirroring with offset translation)
    // public ClusterLinkingConfig ClusterLinking { get; set; } = new();
