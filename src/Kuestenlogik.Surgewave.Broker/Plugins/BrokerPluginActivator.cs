...
public static class BrokerPluginActivator
{
    ...
    /// Plugins signal whether they need a licence via <see cref="IBrokerPlugin.RequiresLicense"/>;
    /// when set, the broker consults <see cref="ILicenseProvider.IsFeatureEnabled"/> with
    /// the plugin's <see cref="IPlugin.FeatureId"/> before activation. Community plugins
    /// (RequiresLicense=false) load unconditionally.
    /// </summary>
        ...
        ILogger? logger = null)
    {
        ...
        foreach (var plugin in discovered)
        {
            ...
            // License gate: plugin self-declares via RequiresLicense whether it needs a
            // licence. The core makes no assumption about which feature ids are licensed
            // — each plugin owns that decision in its own repository.
            if (plugin.RequiresLicense)
            {
                if (license is null || !license.IsFeatureEnabled(plugin.FeatureId))
                {
                    logger?.LogWarning(
                        "Broker plugin '{Plugin}' requires a licence for feature '{Feature}' — skipped (no licence provider or feature not enabled)",
    ...
    public static List<T> Discover<T>() where T : class, IPlugin
    {
        ...
        // Filter to Surgewave assemblies (Kuestenlogik.Surgewave.* host libs + surgewave-*
        // executables + any loaded from plugins/) then delegate the actual type scanning +
        // instantiation to the shared helper. The executable-prefix branch matters for
        // built-in plugins that live inside surgewave-broker.dll itself (Connect / AutoTuning /
        // Schema-Registry); without it Discover<IBrokerPlugin>() would return zero results.
        var surgewaveAssemblies = AppDomain.CurrentDomain.GetAssemblies()
            .Where(a =>
            {
                var name = a.GetName().Name;
                if (name is null) return false;
                return SurgewavePackageConventions.IsSurgewaveHostAssembly(name)
