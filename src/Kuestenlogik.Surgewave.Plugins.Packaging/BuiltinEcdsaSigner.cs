...
/// enterprise scenarios (X.509, HSM, revocation, RFC-3161) use a Sealbolt-backed signer
/// instead.
/// </remarks>
public sealed class BuiltinEcdsaSigner : ISppSigner
{
    ...
    /// <summary>
    /// Creates a signer. Both paths are optional and validated lazily — <see cref="SignAsync"/>
    /// throws if <paramref name="privateKeyPath"/> is missing, <see cref="VerifyAsync"/> throws
    /// if <paramref name="trustedKeysDir"/> is missing. Constructing without either yields a
    /// "no-op" signer suitable for the community-mode default (no signing, no verification).
    /// </summary>
