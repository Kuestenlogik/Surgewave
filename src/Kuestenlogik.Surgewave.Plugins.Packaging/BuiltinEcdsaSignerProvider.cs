...
public sealed class BuiltinEcdsaSignerProvider : ISppSignerProvider
{
    ...
    public ISppSigner Create(IReadOnlyDictionary<string, string> options)
    {
        ...
        if (string.IsNullOrWhiteSpace(privateKeyPath) && string.IsNullOrWhiteSpace(trustedKeysDir))
        {
            throw new ArgumentException(
                "BuiltinEcdsaSignerProvider requires at least one of 'private-key' (signing) or 'trusted-keys-dir' (verification). The direct BuiltinEcdsaSigner constructor accepts both null for a no-op signer in tests; the provider path is for configured use.",
                nameof(options));
        }
