...
builder.Services.AddSingleton(sp => new Kuestenlogik.Surgewave.Broker.ConsumerGroupV2.ConsumerGroupV2Coordinator(
    ...
    sp.GetRequiredService<LogManager>()));
// Neutral contract (#59) resolves to the same singleton so the Kafka adapter depends only on Coordination.
builder.Services.AddSingleton<Kuestenlogik.Surgewave.Coordination.Consumer.IConsumerGroupV2Coordinator>(
    sp => sp.GetRequiredService<Kuestenlogik.Surgewave.Broker.ConsumerGroupV2.ConsumerGroupV2Coordinator>());
builder.Services.AddSingleton(sp => new ConsumerGroupCoordinator(
    ...
    v2Coordinator: sp.GetRequiredService<Kuestenlogik.Surgewave.Coordination.Consumer.IConsumerGroupV2Coordinator>()));
// Neutral contract (#59) resolves to the same singleton so the Kafka adapter (ConsumerGroupApiHandler)
// depends only on Coordination, not the broker-internal coordinator type.
builder.Services.AddSingleton<Kuestenlogik.Surgewave.Coordination.Consumer.IConsumerGroupCoordinator>(
    sp => sp.GetRequiredService<ConsumerGroupCoordinator>());
builder.Services.AddSingleton(sp => new Kuestenlogik.Surgewave.Broker.StreamsGroups.StreamsGroupCoordinator(
    ...
    sp.GetRequiredService<LogManager>()));
// Neutral contract (#59) resolves to the same singleton so the Kafka adapter (StreamsGroupApiHandler)
// depends only on Coordination, not the broker-internal coordinator type.
builder.Services.AddSingleton<Kuestenlogik.Surgewave.Coordination.Streams.IStreamsGroupCoordinator>(
    sp => sp.GetRequiredService<Kuestenlogik.Surgewave.Broker.StreamsGroups.StreamsGroupCoordinator>());
...
var surgewaveBroker = new SurgewaveBroker(config, logManager, recordBatchSerializer, nativeGroupCoordinator, transactionCoordinator, quotaManager, metrics, brokerLogger, connectionHandlers: kafkaConnectionHandlers, pluginDiscovery: pluginDiscoveryForBroker, dlqManager: dlqManager, crossTopicTxnManager: crossTopicTxnManager, kvBucketManager: kvBucketManager, lagCalculator: lagCalculator, loggerFactory: app.Services.GetRequiredService<ILoggerFactory>());
