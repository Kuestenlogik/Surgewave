...
/// <summary>
/// Cap'n Proto deserializer with schema registry integration.
/// Reads Confluent wire format: [0x00 magic][4-byte schema ID][Cap'n Proto frame payload]
/// </summary>
/// <typeparam name="T">The type to deserialize to. Must be a Cap'n Proto generated struct.</typeparam>
public sealed class SchemaRegistryCapnProtoDeserializer<T> : IAsyncDeserializer<T> where T : class, ICapnpSerializable, new()
{
    private readonly CapnProtoSerializerConfig _config;
    private readonly ConcurrentDictionary<int, string> _schemaCache = new();

    /// <summary>
    /// Create a new schema registry Cap'n Proto deserializer.
    /// </summary>
    /// <param name="config">Deserializer configuration.</param>
    public SchemaRegistryCapnProtoDeserializer(CapnProtoSerializerConfig config)
    {
