...
public sealed class KafkaProtocolReader
{
    ...
    /// <summary>
    /// Read tagged fields and dispatch known tags to <paramref name="handler"/>. Unknown tags
    /// (handler returns false) are skipped. The handler is invoked with <c>(tag, payloadSize)</c>
    /// and must read exactly <c>payloadSize</c> bytes from this reader if it returns true;
    /// otherwise the remaining bytes for that tag are skipped automatically.
    /// </summary>
    public void ReadTaggedFields(Func<int, int, bool> handler)
    {
        var tagCount = ReadVarInt();
        for (int i = 0; i < tagCount; i++)
        {
            ...
            var handled = handler(tag, size);
            var consumed = _position - startPos;
            if (!handled || consumed < size)
            {
                Skip(size - consumed);
            }
            else if (consumed > size)
            {
                throw new InvalidDataException($"Tagged field handler for tag {tag} read {consumed} bytes but payload is only {size} bytes");
            }
        }
    }
